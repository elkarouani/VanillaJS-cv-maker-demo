// the dispatcher entry structure
...
export default function domDispatcher(args: DispatcherParams[]) {
	// Initializing the list of html elements variable
    let dom_elements: object[] = [];
	...
	args.forEach((argument) => {
		...
		let dom_element: any = argument.settings && argument.settings.new ? {} : document.getElementById(argument.name);
		...
		if (argument.settings && argument.settings.new) {
			...
			if (argument.settings.brothers) {
				...
				argument.settings.brothers.forEach((brother) => {
					...
					(<any>father_element)!.append(brother);
				});
			}
		}
		...
		if (argument.settings && argument.settings.event) {
			...
			dom_element.addEventListener(argument.settings.event.type, (event: Event) =>
				argument.settings!.event!.callback(event)
			);
		}
